...
function SectionCopy({ section }: { section: ProjectSection }) {
  return (
    <div className="flex h-auto min-h-[50dvh] flex-col justify-between gap-12 overflow-visible bg-[#d9d9d9] p-[clamp(24px,4vw,70px)] text-[#1a1a1a] md:h-full md:min-h-0 md:overflow-y-auto">
      {section.title ? <h2 className="m-0 text-[clamp(24px,3.4vw,48px)] font-light uppercase leading-none">{section.title}</h2> : <span aria-hidden="true" />}
      <div>
...
export function WorkPage() {
  ...
  return (
    <main className="min-h-screen bg-[#1a1a1a] pb-[50px] text-white selection:bg-[#d9ff36] selection:text-[#1a1a1a]">
      ...
      <section className="grid grid-cols-3 bg-[#ff3700] text-black border-b border-white/30 max-md:grid-cols-1">
        <div className="border-r border-white/30 p-[56px_30px] max-sm:p-[36px_18px] max-md:border-b max-md:border-r-0">
          <p className="mb-7 mt-0 text-[11px] uppercase opacity-55">Challenge</p>
          <p className="m-0 text-lg leading-tight">{work.challenge ?? work.statement}</p>
        </div>
          <div className="border-r border-white/30 p-[56px_30px] max-sm:border-b max-sm:border-r-0 max-sm:p-[36px_18px]">
              <p className="mb-7 mt-0 text-[11px] uppercase opacity-55">Client</p>
              <p className="m-0 text-lg leading-tight">{work.client}, {work.year}</p></div>
          <div className="p-[56px_30px] max-sm:p-[36px_18px]">
              <p className="mb-7 mt-0 text-[11px] uppercase opacity-55">Services</p>
      ...
      {work.credits && <section className="border-b border-white/30 p-[76px_0px_96px] max-sm:p-[54px_0px_70px]">
        ...
        <div className="grid grid-cols-1 border-b border-t border-white/30 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6">
          {work.credits.map((credit, creditIndex) => <div key={credit.role} className={`min-h-12 border-white/30 p-5 ${creditCellBorders(creditIndex, work.credits!.length)}`}>
              <p className="m-0 text-[10px] uppercase opacity-50">{credit.role}</p>
              <p className="mb-0 mt-3 text-sm leading-snug">{credit.names.join(', ')}</p></div>)}
        </div>
      </section>}

      <nav className="grid grid-cols-2 max-md:grid-cols-1">
        <Link className="group h-[124px] border-r border-white/30 p-[30px] max-md:border-b max-md:border-r-0 max-sm:p-[18px]" to="/works/$workSlug" params={{ workSlug: previous.slug }}><span className="text-xs uppercase opacity-55">Previous project</span><h2 className="mb-0 mt-4 text-2xl font-light leading-[.9] tracking-[-.06em] transition group-hover:translate-x-2">← {previous.title}</h2></Link>
        <Link className="group h-[124px] bg-[#ff3700] p-[30px] text-[#1a1a1a] max-sm:p-[18px]" to="/works/$workSlug" params={{ workSlug: next.slug }}><span className="text-xs uppercase">Next project</span><h2 className="mb-0 mt-4 text-2xl font-light leading-[.9] transition group-hover:translate-x-2">{next.title} →</h2></Link>
      </nav>
