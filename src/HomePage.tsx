...
const field = `h-[54px] w-full border border-white/30 bg-transparent px-[14px] text-white outline-none placeholder:text-white/65 focus:relative focus:z-10 focus:border-[#d9ff36]`;
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#1a1a1a] pb-[50px] text-white selection:bg-[#d9ff36] selection:text-[#1a1a1a] max-sm:pb-[42px]">
      ...
      <section className="relative mt-[50px] h-[620px] overflow-hidden max-sm:h-[660px]" id="top">
        ...
        <div className="relative z-10 flex h-full flex-col justify-between px-[30px] py-12 text-[clamp(18px,1.58vw,24px)] leading-[1.38] tracking-[-.025em] max-sm:px-[18px] max-sm:py-9 max-sm:text-[17px]">
          <p className="m-0 max-w-[92%]"><strong>NEBBIA Phygital Lab,</strong><br className="max-sm:hidden" /> connessioni e idee che abbattono<br className="max-sm:hidden" /> le barriere tra l’analogico e il digitale.</p>
          <p className="m-0 max-w-[92%] self-end text-right max-sm:max-w-[84%]"><strong>NEBBIA® è un laboratorio phygital,</strong><br className="max-sm:hidden" /> dove creatività, innovazione e comunicazione<br className="max-sm:hidden" /> convergono per supportare aziende e persone.</p>
      ...
      <section className="scroll-mt-[49px]" id="lavori">
        <div className={`grid h-[50px] grid-cols-2 border-y ${line} uppercase`}>
          {(['selected', 'all'] as const).map((view) => (
            <button key={view} type="button" onClick={() => setWorksView(view)} className={`cursor-pointer px-[30px] text-left text-[clamp(18px,1.95vw,25px)] font-extralight transition max-sm:px-[18px] ${view === 'all' ? 'border-l border-white/30' : ''} ${worksView === view ? 'bg-[#f0eee8] text-[#1a1a1a]' : 'bg-[#1a1a1a] text-white'}`}>{view === 'selected' ? 'Selected Work' : 'All Works'}</button>
          ))}
        </div>
        {worksView === 'selected' ? (
          <div className="grid h-[572px] grid-cols-3 max-sm:h-auto max-sm:grid-cols-1">
            {featuredWorks.map((work) => (
              <Link className="group relative min-w-0 overflow-hidden border-r border-white/30 last:border-r-0 max-sm:h-[68vh] max-sm:min-h-[420px] max-sm:border-r-0 max-sm:border-b" to="/works/$workSlug" params={{ workSlug: work.slug }} key={work.slug}>
                <img className="h-full w-full object-cover brightness-[.8] saturate-[.78] transition duration-700 group-hover:scale-[1.04] group-hover:brightness-[.92] group-hover:saturate-100" src={work.image} alt="" />
                <div className="absolute inset-0 flex flex-col justify-between bg-gradient-to-b from-black/45 via-transparent to-black/35 p-[25px_30px_23px] max-sm:p-[21px_18px]">
                  <h2 className="m-0 max-w-[90%] text-[clamp(21px,1.95vw,28px)] font-extralight leading-[1.08] tracking-[-.035em]">{work.title}</h2>
          ...
          <div className="border-b border-white/30">
            {works.map((work, index) => (
              <Link className="group grid min-h-[56px] grid-cols-[56px_minmax(250px,1.1fr)_minmax(300px,1fr)_34px] items-center border-b border-white/30 px-[30px] transition last:border-b-0 hover:bg-[#f0eee8] hover:text-[#1a1a1a] max-sm:min-h-[84px] max-sm:grid-cols-[34px_1fr_24px] max-sm:px-[18px]" to="/works/$workSlug" params={{ workSlug: work.slug }} key={work.slug}>
                <span className="text-[10px] opacity-50">{String(index + 1).padStart(2, '0')}</span>
                <h2 className="m-0 text-[clamp(19px,1.9vw,26px)] font-extralight leading-[1.05] tracking-[-.03em]">{work.title}</h2>
                <p className="m-0 text-xs opacity-70 max-sm:hidden">{work.services}</p>
                <b className="text-2xl font-extralight transition group-hover:rotate-45" aria-hidden="true">＋</b>
              </Link>
            ))}
          </div>
        )}
      ...
      <section className="scroll-mt-[49px]" id="contatti">
        ...
        <div className="grid min-h-[760px] grid-cols-2 max-lg:grid-cols-1">
          <aside className="flex min-w-0 flex-col border-r border-white/30 p-[48px_30px_38px] max-lg:min-h-[420px] max-lg:border-b max-lg:border-r-0 max-sm:min-h-[390px] max-sm:px-[18px]">
            <p className="mb-[34px] mt-0 text-[11px] uppercase tracking-[.06em]">Informazioni generali</p>
            <address className="text-[clamp(20px,2.1vw,29px)] font-extralight not-italic leading-[1.26] tracking-[-.03em]">Via dell&apos;Innovazione digitale, 3<br />26100 Cremona, Italia<br />P.I. 01618080194</address>
            <a className="mt-auto break-words text-[clamp(24px,4.3vw,62px)] leading-[.95] tracking-[-.055em]" href="mailto:info@nebbialab.it">info@nebbialab.it <sup className="text-[.4em]">↗</sup></a>
          </aside>
          <div className="p-[46px_30px_38px] max-sm:px-[18px]">
            ...
            <form onSubmit={handleSubmit}>
              <div className="grid grid-cols-2 max-sm:grid-cols-1"><input className={field} name="nome" placeholder="Nome" aria-label="Nome" required /><input className={`${field} border-l-0 max-sm:-mt-px max-sm:border-l`} name="cognome" placeholder="Cognome" aria-label="Cognome" required /></div>
              <div className="-mt-px grid grid-cols-2 max-sm:grid-cols-1"><input className={field} name="azienda" placeholder="Company" aria-label="Azienda" /><input className={`${field} border-l-0 max-sm:-mt-px max-sm:border-l`} name="email" type="email" placeholder="Email" aria-label="Email" required /></div>
              <select className={`${field} -mt-px appearance-none`} name="oggetto" aria-label="Scegli l'oggetto"><option className="text-[#1a1a1a]">Info Generali</option><option className="text-[#1a1a1a]">Preventivi</option><option className="text-[#1a1a1a]">Partnership</option></select>
              <textarea className={`${field} -mt-px h-[190px] resize-y py-[15px]`} name="messaggio" placeholder="Message" aria-label="Messaggio" required />
              <button className="-mt-[7px] flex h-[54px] w-full cursor-pointer items-center justify-between border border-white bg-white px-[15px] uppercase text-[#1a1a1a] transition hover:border-[#d9ff36] hover:bg-[#d9ff36]" type="submit">Invia <span className="text-xl" aria-hidden="true">↗</span></button>
      ...
      <section className="scroll-mt-[49px]" id="lavora-con-noi">
        ...
        <div className="grid min-h-[540px] grid-cols-2 max-lg:grid-cols-1">
          <div className="p-[48px_30px] max-lg:min-h-[430px] max-sm:px-[18px]"><p className="mb-[34px] mt-0 text-[11px] uppercase tracking-[.06em]">Posizioni aperte</p><p className="m-0 text-[clamp(25px,3vw,44px)] leading-[1.08] tracking-[-.045em]">Vuoi far parte del nostro team?<br />Consulta le posizioni aperte<br />e candidati per avere l’opportunità<br />di partecipare a un colloquio con noi.</p></div>
          <a className="career-cta group relative grid min-h-[540px] place-items-center overflow-hidden bg-[#f0eee8] text-center uppercase text-[#1a1a1a] transition hover:bg-[#d9ff36] max-sm:min-h-[100vw]" href="https://nebbialab.it/candidati-ora/" target="_blank" rel="noreferrer">
            <i className="absolute aspect-square w-[min(75%,390px)] rounded-full border border-[#1a1a1a] transition duration-500 group-hover:scale-80 group-hover:bg-[#1a1a1a]" />
            <span className="relative z-10 text-[clamp(36px,5.3vw,74px)] leading-[.87] tracking-[-.07em] transition group-hover:text-white">Candidati<br />ora</span><b className="absolute right-[30px] top-6 z-10 text-[32px] font-extralight transition group-hover:text-white" aria-hidden="true">↗</b>
          </a>
        </div>
      ...
      <footer className="fixed inset-x-0 bottom-0 z-[60] flex h-[50px] items-center gap-3 border-t border-white/30 bg-[#1a1a1a] px-[30px] text-xs max-sm:h-[42px] max-sm:gap-2 max-sm:px-[18px] max-sm:text-[10px]">
        <span>Cremona (IT)</span><i className="h-3 w-px bg-white/45" /><span>Time: <time>{time || '--:--'}</time></span><i className="h-3 w-px bg-white/45" /><span className="flex items-center gap-1.5"><b className="size-2 rounded-full bg-[#d9ff36] shadow-[0_0_12px_#d9ff36]" /> Clear</span><small className="ml-auto text-[10px] opacity-50 max-sm:hidden">© {new Date().getFullYear()} Nebbia Phygital Lab</small>
      </footer>
    </main>
  );
}
...
function SectionTitle({ title, detail }: { title: string; detail: string }) {
  return <header className="flex h-[51px] items-center justify-between border-y border-white/30 px-[30px] uppercase max-sm:px-[18px]"><h2 className="m-0 text-[clamp(20px,2vw,26px)] font-extralight">{title}</h2><span className="text-[10px] tracking-[.11em] opacity-55">{detail}</span></header>;
}

function FooterCol({ title, children }: { title: string; children: React.ReactNode }) {
  return <div className="flex min-w-0 flex-col border-r border-white/30 p-[28px_30px] text-xs last:border-r-0 max-lg:border-b max-sm:min-h-[125px] max-sm:border-r-0 max-sm:px-[18px]"><p className="mb-6 mt-0 uppercase opacity-45">{title}</p>{children}</div>;
}
